...
function Volume({ link }: { link: WanLink }) {
  ...
  const gb = `${(used / 1e9).toFixed(used < 1e10 ? 2 : 1)} GB`; // dezimal wie bei Mobilfunk-Tarifen
  if (!link.monthly_limit_gb) return <span className="text-xs">{gb}</span>;
  const pct = (100 * used) / (link.monthly_limit_gb * 1e9);
  const color = pct >= 100 ? "bg-red-500" : pct >= 80 ? "bg-amber-500" : "bg-emerald-500";
  return (
    <div className="min-w-[8rem] text-xs">
      <div>{gb} / {link.monthly_limit_gb} GB <span className="text-slate-500">({pct.toFixed(0)} %)</span></div>
      <div className="mt-1 h-1.5 rounded bg-slate-200"><div className={`h-1.5 rounded ${color}`} style={{ width: `${Math.min(pct, 100)}%` }} /></div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <Card title="Status der WAN-Verbindungen" actions={editable && <Button variant="secondary" disabled={busy} onClick={() => void run(async () => { await api.post(`/devices/${device.id}/wan/apply`); await wan.reload(); })}>Erneut anwenden</Button>}>
        ...
        <Table head={["WAN", "Interface", "Gateway", "Status", "Aktiv", "Latenz", "Verlust", "Volumen (Monat)", "Letzter Wechsel", ""]} empty={wan.data.links.length === 0}>
          {wan.data.links.map((l) => (
            <tr key={l.id}>
              ...
              <td className="px-3 py-2"><Volume link={l} /></td>
            ...
            {draft.links.map((l, i) => (
              <div key={i} className="rounded-lg border border-slate-200 p-4">
                ...
                <div className="grid gap-3 md:grid-cols-4">
                  ...
                  <Input label="Monatslimit (GB, optional)" type="number" min={0} step="any" value={l.monthly_limit_gb ?? ""} onChange={(e) => setLink(i, { monthly_limit_gb: e.target.value ? Number(e.target.value) : null })} />
                </div>
              </div>
            ))}
          </div>
