  ...
  return (
    <>
      <PageHeader
        ...
        actions={can("technician") && <Button disabled={needsTenant} title={needsTenant ? "Bitte zuerst Mandant wählen" : ""} onClick={() => setEdit({ lan_subnets: [], is_mesh_hub: false })} icon="plus">Standort</Button>}
      />
      <Card>
...
function SiteModal({ site, onClose, onSaved }: { site: Partial<Site>; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <Modal open onClose={onClose} title={site.id ? "Standort bearbeiten" : "Neuer Standort"}
      footer={<>
        {site.id && can("admin") && <Button type="button" variant="danger-outline" icon="trash" className="mr-auto" onClick={() => confirm("Standort löschen?") && void run(async () => { await api.del(`/sites/${site.id}`); onSaved(); })}>Löschen</Button>}
        <Button type="button" variant="secondary" onClick={onClose}>Abbrechen</Button>
        <Button form="site-form" disabled={busy}>Speichern</Button>
      </>}>
      <form id="site-form" className="space-y-3" onSubmit={(e) => { e.preventDefault(); void save(); }}>
        ...
        <Textarea label="LAN-Netze" hint="Eines pro Zeile, z. B. 192.168.10.0/24" rows={3} value={f.subnets} onChange={(e) => setF({ ...f, subnets: e.target.value })} />
